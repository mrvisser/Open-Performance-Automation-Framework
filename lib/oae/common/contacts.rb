...
class Contacts
  ...
  def my_contacts(username)
    request = SessionUtil.new(@session).create_txn_reqs('my_contacts')
	...
	def invite(inviter_username, invitee_username)
	  request = SessionUtil.new(@session).create_txn_reqs('invite_contact')
	...
	def accept(inviter_username, invitee_username)
	  request = SessionUtil.new(@session).create_txn_reqs('accept_contact')
